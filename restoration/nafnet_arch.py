"""
NAFNet architecture (Nonlinear Activation Free Network)
Reference: Chen et al., "Simple Baselines for Image Restoration", ECCV 2022.
Self-contained pure PyTorch implementation (zero basicsr dependency).
"""
import torch
...
class NAFNet(nn.Module):
    def __init__(self, img_channel=3, width=64, middle_blks_num=12, enc_blks=[2, 2, 4, 8], dec_blks=[2, 2, 2, 2]):
        super().__init__()
        ...
        self.middle_blks = nn.Sequential(
            *[NAFBlock(chan) for _ in range(middle_blks_num)]
        )
